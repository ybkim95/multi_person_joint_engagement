...
def main():
    ...
    #print("[INFO] results[0]:", results[0].shape)


    # 1) original video (256x256)
    try:
        from moviepy.editor import ImageSequenceClip
    except ImportError:
        raise ImportError('Please install moviepy to enable output file.')

    ybk_frames_batches = (orig_inputs[0] * 255.).numpy().astype(np.uint8)
    print("[INFO] ybk_frames_batches:", ybk_frames_batches.shape)
    ybk_frames = ybk_frames_batches.reshape(-1, *ybk_frames_batches.shape[-3:])
    print("[INFO] ybk_frames:", ybk_frames.shape)
 
    ybk_frame_list = list(ybk_frames)
    ybk_frame_list = _resize_frames(
        ybk_frame_list,
        args.target_resolution,
        interpolation=args.resize_algorithm)
 
    ybk_video_clips = ImageSequenceClip(ybk_frame_list, fps=args.fps)
    #print("[INFO] ybk_video_clips:", ybk_video_clips.shape)
 
    # save video
    ybk_video_clips.write_videofile(args.out_filename.split(".")[0] + "_orig.mp4", remove_temp=True)


    # 2) heatmap only video (256x256)
    try:
        from moviepy.editor import ImageSequenceClip
    except ImportError:
        raise ImportError('Please install moviepy to enable output file.')

    heat_frames_batches = (heat_inputs[0] * 255.).numpy().astype(np.uint8)
    print("[INFO] heat_frames_batches:", heat_frames_batches.shape)
    heat_frames = heat_frames_batches.reshape(-1, *heat_frames_batches.shape[-3:])
    print("[INFO] heat_frames:", heat_frames.shape)

    heat_frame_list = list(heat_frames)
    heat_frame_list = _resize_frames(
        heat_frame_list,
        args.target_resolution,
        interpolation=args.resize_algorithm)

    heat_video_clips = ImageSequenceClip(heat_frame_list, fps=args.fps)

    # save video
    heat_video_clips.write_videofile(args.out_filename.split(".")[0] + "_heat.mp4", remove_temp=True)
    ...
    if args.out_filename is not None:
        ...
        print("[INFO] frames_batches:", frames_batches.shape)
        frames = frames_batches.reshape(-1, *frames_batches.shape[-3:])
        print("[INFO] frames:", frames.shape)
