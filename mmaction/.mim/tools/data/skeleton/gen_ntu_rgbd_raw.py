...
def read_skeleton_filter(file):
    ...
    with open(file, 'rb') as f:
        ...
        skeleton_sequence = {}
        skeleton_sequence['num_frame'] = int(len(data))
        skeleton_sequence['frameInfo'] = []

        for t in range(skeleton_sequence['num_frame']):
            frame_info = {}
            frame_info['numBody'] = int(len(data))
            frame_info['bodyInfo'] = []
...
def gendata(data_path,
    ...
    f = open("/u/ybkim95/mmaction2/data/total_labels/3s_cc_total.txt", "r")
    lines = f.readlines()
    labels = {}
    for line in lines:
        tmp  = line.split()
        ...
        key = "_".join(vpath.split("/")[-2:]).split(".")[0]
        labels[key] = label
   
    sample_name = []
    sample_label = []
    data_path = '/u/ybkim95/mmaction2/extracted_skeletons1'
    for dir1 in os.listdir(data_path):
        ...
        for dir2 in os.listdir(fpath1):
            ...
            filename = "{}/{}".format(dir1, dir2)
            sample_name.append(filename)

            action_class = int(labels["{}_{}".format(dir1, dir2.split(".")[0])])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='data/posec3d')
    parser.add_argument(
        '--ignored-sample-path',
        type=str,
        default='NTU_RGBD_samples_with_missing_skeletons.txt')
    parser.add_argument(
        '--out-folder', type=str, default='data/posec3d/ntu60_output')
